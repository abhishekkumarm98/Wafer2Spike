...
class Wafer2Spike(nn.Module):
    ...
    def __init__(self, numClasses, dropout_fc, spike_ts, device, params):
        ...
        self.Spk_conv3_w_vdecay = nn.Parameter(torch.ones(1, 64, 3, 3, device=self.device) * self.vdecay)

        # Synaptic current decay for Spiking-Based Convolutional Layer3
        self.Spk_conv3_w_scdecay = nn.Parameter(torch.ones(1, 64, 3, 3, device=self.device) * self.scdecay)
        ...
        self.Spk_conv3 = CurrentBasedLIF(nn.Conv2d(64, 64, (7, 7), stride=2, bias=True), pseudo_grad_ops, 
    ...
    def forward(self, input_data, states):
        ...
        conv_spk_enc_state, Spk_conv1_state, Spk_conv2_state, Spk_conv3_state, Spk_fc_state = states[0], states[1], states[2], states[3], states[4]
        ...
        for step in range(self.spike_ts):
            ...
            # Flattening
            flattened_spike = Spk_conv3_spike.view(batch_size, -1)
...
class CurrentBasedSNN(nn.Module):
    ...
    def forward(self, input_data):
        ...
        Spk_conv3_current = torch.zeros(batch_size, 64, 3, 3, device=self.device)
        Spk_conv3_volt = torch.zeros(batch_size, 64, 3, 3, device=self.device)
        Spk_conv3_spike = torch.zeros(batch_size, 64, 3, 3, device=self.device)
        ...
        states = (conv_spk_enc_state, Spk_conv1_state, Spk_conv2_state, Spk_conv3_state, Spk_fc_state)
