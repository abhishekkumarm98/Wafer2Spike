...
class CurrentBasedSNN(nn.Module):
    ...
    def forward(self, input_data):
        ...
        Spk_conv1_current = torch.zeros(batch_size, 64, 12, 12, device=self.device)
        Spk_conv1_volt = torch.zeros(batch_size, 64, 12, 12, device=self.device)
        Spk_conv1_spike = torch.zeros(batch_size, 64, 12, 12, device=self.device)
        Spk_conv1_state = (Spk_conv1_spike, Spk_conv1_current, Spk_conv1_volt)
        
        # For Spiking-Based Convolutional Layer2
        Spk_conv2_current = torch.zeros(batch_size, 64, 3, 3, device=self.device)
        Spk_conv2_volt = torch.zeros(batch_size, 64, 3, 3, device=self.device)
        Spk_conv2_spike = torch.zeros(batch_size, 64, 3, 3, device=self.device)
